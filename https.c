...
static void ssl_load_private_certificates(SSL_CTX *ctx)
{
	...
	if (errs) {
		...
		if (!ses) {
			...
			msg_box(ses->term, getml(err_str, numfail_str, NULL), TEXT_(T_SSL_ERROR), AL_CENTER | AL_EXTD_TEXT, TEXT_(T_ERROR_INITIALIZING_BUILT_IN_CERTIFICATES), ": ", err_str, ", ", TEXT_(T_FAILED), " ", numfail_str, NULL, NULL, 1, TEXT_(T_CANCEL), NULL, B_ENTER | B_ESC);
		}
	}
}
#endif
...
static void set_session_cache_entry(SSL_CTX *ctx, unsigned char *host, int port, SSL_SESSION *s)
{
	...
	if (!s)
		return;
	sce = mem_alloc(sizeof(struct session_cache_entry) + strlen(cast_const_char host));
